...
# Added option for "Average Plant Average YTD" as an alternative view call in selection box
...
# Manage segment filter routing rules
...
if available_nodes:
    ...
    # 2. Assign standard palette color configurations
    base_colors = px.colors.qualitative.Plotly
    color_map = {}
    for i, node in enumerate(available_nodes):
        color_map[node] = base_colors[i % len(base_colors)]
        
    # 3. Base Chart Generation
    fig_line = px.line(
        df_melted_monthly,
        x="Month",
    ...
    # 4. FLAT HORIZONTAL INDIVIDUAL YEARLY BASELINES
    ...
    # 5. Macro Segment Average Line (Mean calculation month over month)
    ...
    # 6. FIX: Add the "Average Plant Average YTD" cumulative trace condition
    # Compiles the flat horizontal aggregate of all selected individual yearly means
    ...
    fig_line.add_trace(go.Scatter(
        ...
        line=dict(color="#f43f5e", width=3.5, dash="dashdot"), # Prominent Rose/Pink color trace
        showlegend=True
    ))
    ...
    with col_legend:
        ...
        st.write("* ⚠️ **Rose Dash-Dot:** Flat Master Horizon Baseline (**Average Plant Average YTD**).")
